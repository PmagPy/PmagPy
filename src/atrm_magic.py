...
def main():
    ...
    # find the data from the meas_data file for this sample
        ...
        if len(data) > 5:
            ...
            RmagSpecRec[
                "magic_experiment_names"] = RmagSpecRec[
                "rmag_anisotropy_name"] + ":ATRM"
            RmagSpecRec["er_citation_names"] = "This study"
            RmagResRec["rmag_result_name"] = data[
                0]["er_specimen_name"] + ":ATRM"
            ...
            if "magic_instrument_codes" in data[0].keys():
                RmagSpecRec["magic_instrument_codes"] = data[
                    0]["magic_instrument_codes"]
            else:
    ...
    #
        ...
        # if BX isn't just one measurement or one in between every infield
        # step, just assume it is zero
        elif len(BX) != len(X):
