...
def main():
    ...
    # field
        ...
        for rec in meas_data:
            ...
            # looking for chi measurement
            if rec['magic_experiment_name'] == e and "LP-X" in meths:
        ...
        if len(X) > 2:  # if there are any data to plot, continue
            ...
            for f in Fs:   # step through frequencies sequentially
                ...
                for kk in range(len(X)):  # hunt through all the data
                    # select data with given freq and field
                    if F[kk] == f and B[kk] == b:
            ...
            if len(XT) > 1:  # if there are any temperature dependent data
                ...
                plotnum += 1  # increment plot number
            if '-i' in sys.argv:
                # print list of temperatures available
            ...
            while cont != "q":
                ...
                for kk in range(len(X)):  # hunt through the data
                    # if temperature and field match,
                    if T[kk] == Ts[Tind] and B[kk] == b:
                        XF.append([X[kk], F[kk]])  # append the data
                if len(XF) > 1:  # if there are any data to plot
                    # if not already initialized, initialize plot
                    if FTinit == 0:
                ...
                if '-i' in sys.argv:
                    # just in case you forgot, print out a new list of
                    # temperatures
                    for ind in range(len(Ts)):
                        print ind, int(Ts[ind])
                    # ask for new temp
