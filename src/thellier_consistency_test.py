...
def Thellier_consistency_test(
        WD, Data, Data_hierarchy, criteria_fixed_paremeters_file,
        optimizer_group_file_path, optimizer_functions_path, beta_range, frac_range):
    """
    ...
    def dir2cart(d):
         # converts list or array of vector directions, in degrees, to array of
         # cartesian coordinates, in x,y,z
        ...
        if len(d.shape) > 1:  # array of vectors
            ...
            if len(d) == 3:
                ...
                ints = array([1.])
        cart = array(
            [ints * cos(decs) * cos(incs),
             ints * sin(decs) * cos(incs),
             ints * sin(incs)]).transpose()
        return cart

    def calculate_ftest(s, sigma, nf):
        chibar = (s[0][0] + s[1][1] + s[2][2]) / 3.
        t = array(linalg.eigvals(s))
        F = 0.4 * (t[0] ** 2 + t[1] ** 2 + t[2] ** 2 - 3 * chibar ** 2) / \
            (float(sigma) ** 2)
    ...
    logfile = open(
        WD +
        "/consistency_test/consistency_test_optimizer.log",
        'w')
    start_time = time.time()
    accept_new_parameters = {}
    criteria_specimen_list = [
        'specimen_int_n', 'specimen_int_ptrm_n', 'specimen_f', 'specimen_fvds', 'specimen_frac', 'specimen_gmax', 'specimen_b_beta', 'specimen_scat',
        'specimen_dang', 'specimen_drats', 'specimen_int_mad', 'specimen_md', 'specimen_g', 'specimen_q']
    criteria_sample_list = [
        'sample_int_n',
        'sample_int_sigma_uT',
    ...
    thellier_optimizer_master_file = gzip.open(
        WD +
        '/consistency_test/consistency_test_log.txt.gz',
        'wb')

    String = "er_specimen_name\t" + "t_min\t" + \
        "t_max_\t" + "specimen_int_uT\t"
    ...
    Optimizer_results_file = open(
        WD +
        "/consistency_test/consistency_test_results.txt",
        'w')
...
# if OPTIMIZE_BY_LIST:
    ...
    for line in fin.readlines():
        ...
        if "sample_int" in tmp_data.keys() and tmp_data['sample_int'] != "":
            try:
                # convert form T to uT
                samples_expected_intensity[sample] = float(
                    tmp_data['sample_int']) * 1e6
            except:
                pass
        ...
        for start in range(0, len(t_Arai) - n_min + 1):
            for end in range(start + n_min - 1, len(t_Arai)):
                ...
                if tmin not in z_temperatures:
                    logfile.write(
                        "-E- ERROR: specimen %s temperature %f appears in Arai plot but not in Zijderveld plot\n" %
                        (s, tmin))
                    continue
                ...
                [eigenvalues, eigenvectors] = linalg.eig(cov(M))
                ...
                # york sigma
                york_sigma = sqrt(
                    (2 * sum(y_err ** 2) - 2 * york_b * sum(x_err * y_err)) / ((n - 2) * sum(x_err ** 2)))
                ...
                # intersect of the dashed square and the horizontal dahed line
                # next to delta-y-5 in figure 7, Coe (1978)
                ...
                pars["specimen_int"] = -1 * \
                    pars['lab_dc_field'] * pars["specimen_b"]
                ...
                for k in range(len(Data[s]['ptrm_checks_temperatures'])):
                    if Data[s]['ptrm_checks_temperatures'][k] < pars["measurement_step_max"] and Data[s]['ptrm_checks_temperatures'][k] in t_Arai:
                        x_ptrm_check_in_0_to_end.append(
                            Data[s]['x_ptrm_check'][k])
                        y_ptrm_check_in_0_to_end.append(
                            Data[s]['y_ptrm_check'][k])
                        x_Arai_index = t_Arai.index(
                            Data[s]['ptrm_checks_temperatures'][k])
                        x_Arai_compare.append(x_Arai[x_Arai_index])
                        if Data[s]['ptrm_checks_temperatures'][k] >= pars["measurement_step_min"]:
                            x_ptrm_check_in_start_to_end.append(
                                Data[s]['x_ptrm_check'][k])
                            y_ptrm_check_in_start_to_end.append(
                                Data[s]['y_ptrm_check'][k])
                ...
                x_ptrm_check_in_start_to_end = array(
                    x_ptrm_check_in_start_to_end)
                y_ptrm_check_in_start_to_end = array(
                    y_ptrm_check_in_start_to_end)

                DRATS = 100 * \
                    (abs(sum(x_ptrm_check_in_0_to_end - x_Arai_compare))
                     / (x_Arai[end]))
                ...
                for k in range(len(Data[s]['tail_check_temperatures'])):
                    if Data[s]['tail_check_temperatures'][k] in t_Arai:
                        if Data[s]['tail_check_temperatures'][k] <= pars["measurement_step_max"] and Data[s]['tail_check_temperatures'][k] >= pars["measurement_step_min"]:
                            x_tail_check_start_to_end.append(
                                Data[s]['x_tail_check'][k])
                            y_tail_check_start_to_end.append(
                                Data[s]['y_tail_check'][k])
                ...
                max_FRAC_gap = max(
                    vector_diffs_segment / sum(vector_diffs_segment))

                pars['specimen_frac'] = FRAC
                pars['specimen_gmax'] = max_FRAC_gap

                # avoid pathological cases when specimen_frac is very small
                # (less than 10%)
                ...
                if "AniSpec" in Data[s].keys():
                    pars["AC_WARNING"] = ""
                    # if both aarm and atrm tensor axist, try first the atrm.
                    # if it fails use the aarm.
                    ...
                    for TYPE in TYPES:
                        ...
                        S_matrix[0,
                                 0] = Data[s]['AniSpec'][TYPE]['anisotropy_s1']
                        S_matrix[1,
                                 1] = Data[s]['AniSpec'][TYPE]['anisotropy_s2']
                        S_matrix[2,
                                 2] = Data[s]['AniSpec'][TYPE]['anisotropy_s3']
                        S_matrix[0,
                                 1] = Data[s]['AniSpec'][TYPE]['anisotropy_s4']
                        S_matrix[1,
                                 0] = Data[s]['AniSpec'][TYPE]['anisotropy_s4']
                        S_matrix[1,
                                 2] = Data[s]['AniSpec'][TYPE]['anisotropy_s5']
                        S_matrix[2,
                                 1] = Data[s]['AniSpec'][TYPE]['anisotropy_s5']
                        S_matrix[0,
                                 2] = Data[s]['AniSpec'][TYPE]['anisotropy_s6']
                        S_matrix[2,
                                 0] = Data[s]['AniSpec'][TYPE]['anisotropy_s6']

                        Data[s]['AniSpec'][TYPE]['anisotropy_n'] = int(
                            float(Data[s]['AniSpec'][TYPE]['anisotropy_n']))

                        this_specimen_f_type = Data[s]['AniSpec'][TYPE][
                            'anisotropy_type'] + "_" + "%i" % (int(Data[s]['AniSpec'][TYPE]['anisotropy_n']))
                    ...
                    # anistropy tensor to identity matrix
                        ...
                        if Data[s]['AniSpec'][TYPE]['anisotropy_alt'] != "":
                            if float(Data[s]['AniSpec'][TYPE]['anisotropy_alt']) > float(accept_new_parameters['anisotropy_alt']):
                                S_matrix = identity(3, 'f')
                                pars["AC_WARNING"] = pars["AC_WARNING"] + "%s tensor fails alteration check: %.1f%% > %.1f%%; " % (
                                    TYPE,
                                    float(
                                        Data[
                                            s]['AniSpec'][TYPE]['anisotropy_alt']),
                                    float(accept_new_parameters['anisotropy_alt']))
                        ...
                        if TYPE == 'AARM' and red_flag == False:
                            ...
                            # print 'specimen %s AARM fails ftest, try
                            # ATRM'%(s)
                            pass

                    TRM_anc_unit = array(pars['specimen_PCA_v1']) / sqrt(pars['specimen_PCA_v1']
                                                                         [0] ** 2 + pars['specimen_PCA_v1'][1] ** 2 + pars['specimen_PCA_v1'][2] ** 2)
                    B_lab_unit = dir2cart(
                        [Data[s]['Thellier_dc_field_phi'],
                         Data[s]['Thellier_dc_field_theta'],
                            1])
                    B_lab_unit = array([0, 0, -1])
                    Anisotropy_correction_factor = linalg.norm(
                        dot(inv(S_matrix),
                            TRM_anc_unit.transpose())) * norm(dot(S_matrix,
                                                                  B_lab_unit))
                    pars[
                        "Anisotropy_correction_factor"] = Anisotropy_correction_factor
                    pars["AC_specimen_int"] = pars["Anisotropy_correction_factor"] * \
                        float(pars["specimen_int"])
                    pars["AC_anisotropy_type"] = Data[s][
                        'AniSpec'][TYPE]["anisotropy_type"]
                    pars["specimen_int_uT"] = float(
                        pars["AC_specimen_int"]) * 1e6
                ...
                if 'NLT_parameters' in Data[s].keys():
                    ...
                    if ((abs(b) * Fa) / alpha) < 1.0:
                        ...
                        if "AC_specimen_int" in pars.keys():
                            pars["NLT_specimen_correction_factor"] = Banc_NLT / \
                                float(pars["AC_specimen_int"])
                        else:
                            pars["NLT_specimen_correction_factor"] = Banc_NLT / \
                                float(pars["specimen_int"])
                    else:
                        logfile.write(
                            "-W- WARNING: problematic NLT mesurements specimens %s. Cant do NLT calculation. check data\n" %
                            s)
                ...
                #--------------------------------------------------------------
                String = s + "\t" + "%s\t" % pars["measurement_step_min"] + \
                    "%s\t" % pars["measurement_step_max"] + \
                    "%s\t" % pars["specimen_int_uT"]
                for crit in criteria_specimen_list:
                    ...
                    for pars in thellier_optimizer_master_table[specimen]:
                        ...
                        for k in range(len(Data[specimen]['ptrm_checks_temperatures'])):
                            ...
                            if Data[specimen]['ptrm_checks_temperatures'][k] >= pars["measurement_step_min"] and Data[specimen]['ptrm_checks_starting_temperatures'][k] <= pars["measurement_step_max"]:
                                x_ptrm_check_for_SCAT.append(
                                    Data[specimen]['x_ptrm_check'][k])
                                y_ptrm_check_for_SCAT.append(
                                    Data[specimen]['y_ptrm_check'][k])
                            ...
                            # the ateration occured between 450 and 500, we
                            # dont know when.
                        ...
                        for k in range(len(Data[specimen]['tail_check_temperatures'])):
                            ...
                            if Data[specimen]['tail_check_temperatures'][k] >= pars["measurement_step_min"] and Data[specimen]['tail_checks_starting_temperatures'][k] <= pars["measurement_step_max"]:
                                x_tail_check_for_SCAT.append(
                                    Data[specimen]['x_tail_check'][k])
                                y_tail_check_for_SCAT.append(
                                    Data[specimen]['y_tail_check'][k])
                        x_tail_check_for_SCAT = array(x_tail_check_for_SCAT)
                        y_tail_check_for_SCAT = array(y_tail_check_for_SCAT)
                        ...
                        b_beta_threshold = accept_new_parameters[
                            'specimen_b_beta']

                        two_sigma_beta_threshold = 2 * b_beta_threshold
                        two_sigma_slope_threshold = abs(
                            two_sigma_beta_threshold *
                            b)

                        # a line with a  shallower  slope  (b + 2*beta*b)
                        # passing through the center of mass
                        b1 = b + two_sigma_slope_threshold
                        a1 = cm_y - b1 * cm_x

                        # bounding line with steeper  slope (b - 2*beta*b)
                        # passing through the center of mass
                        ...
                        if (sum(check_1) + sum(check_2)) > 0:
                            thellier_optimizer_master_file.write(
                                "key=%s - specimen %s, tmin,tmax =(%.0f,%.0f) fail on scat\n" %
                        ...
                        if len(x_ptrm_check_for_SCAT) > 0:
                            ...
                            if (sum(check_1) + sum(check_2)) > 0:
                                thellier_optimizer_master_file.write(
                                    "key=%s - specimen %s, tmin,tmax =(%.0f,%.0f) fail on ptrm scat\n" %
                                    (Key, specimen, float(pars["measurement_step_min"]) - 273, float(pars["measurement_step_max"]) - 273))
                                continue
                        ...
                        if len(x_tail_check_for_SCAT) > 0:
                            ...
                            if (sum(check_1) + sum(check_2)) > 0:
                                thellier_optimizer_master_file.write(
                                    "key=%s - specimen %s, tmin,tmax =(%.0f,%.0f) fail on tail scat\n" %
                                    (Key, specimen, float(pars["measurement_step_min"]) - 273, float(pars["measurement_step_max"]) - 273))
                                continue
    ...
    #-------------------------------------------------
    logfile.write(
        "-I- Looping now through frac and beta ans finding the best means\n")
    ...
    for frac in frac_range:
        for beta in beta_range:
            ...
            thellier_optimizer_master_file.write(
                "-I- calulating best sample means, %s" %
                Key)
            ...
            for sample in Optimizer_data[Key].keys():
                                    ...
                                    if "AC_WARNING" in pars.keys():
                                        if "TRM" in pars["AC_WARNING"] and  pars["AC_anisotropy_type"] == "ATRM" \
                                           or "ARM" in pars["AC_WARNING"] and pars["AC_anisotropy_type"] == "AARM":
                                            continue
                                    AC_correction_factor = max(
                                        AC_correction_factor,
                                        pars["Anisotropy_correction_factor"])
                            if AC_correction_factor != 0:
                                aniso_corrections.append(
                                    abs(1. - float(AC_correction_factor)))
                        # 0.10:
                        if mean(aniso_corrections) > float(accept_new_parameters['sample_aniso_threshold_perc']) / 100:
                            tmp_B_specimens = copy.deepcopy(
                                B_specimens[sample])
                            ...
                            for specimen in Optimizer_data[Key][sample].keys():
                                ...
                                if ignore_specimen:
                                    warning_messeage = warning_messeage + \
                                        "-W- WARNING: specimen %s is excluded from sample %s because it doesnt have anisotropy correction, and other specimens are very anistropic\n" % (
                                            specimen, sample)
                            ...
                            if len(tmp_B_specimens.keys()) >= accept_new_parameters['sample_int_n']:
                                ...
                                sample_acceptable_min, sample_acceptable_max = find_sample_min_max_interpretation(
                                    tmp_B_specimens, accept_new_parameters)

                                sample_int_interval_uT = sample_acceptable_max - \
                                    sample_acceptable_min
                                sample_int_interval_perc = 100 * \
                                    ((sample_acceptable_max - sample_acceptable_min)
                                     / best_mean)

                                # print "tmp_B_specimens 22",tmp_B_specimens
                                # print "B_specimens 22",B_specimens[sample]

                                # check if interpretation pass criteria (if yes
                                # ignore the specimens). if no, keep it the old
                                # way:
                ...
                if len(B_specimens[sample].keys()) >= float(accept_new_parameters['sample_int_n_outlier_check']):
                    ...
                    for specimen in all_specimens:
                        ...
                        for specimen_b in all_specimens:
                            if specimen_b == specimen:
                                continue
                            B_min_array.append(
                                min(B_specimens[sample][specimen_b]))
                            B_max_array.append(
                                max(B_specimens[sample][specimen_b]))
                    ...
                    if len(exclude_specimens_list) > 1:
                        thellier_optimizer_master_file.write(
                            "-I- check outlier sample =%s; frac=%f,beta=%f,: more than one specimen can be excluded.\n" %
                            (sample, frac, beta))
                        exclude_specimens_list = []
                    ...
                    if len(exclude_specimens_list) == 1:
                        ...
                        thellier_optimizer_master_file.write(
                            "-W- WARNING: frac=%f,beta=%f, specimen %s is exluded from sample %s because is is an outlier.\n " %
                ...
                if len(B_specimens[sample].keys()) >= accept_new_parameters['sample_int_n']:
                    ...
                    Best_interpretations, best_mean, best_std = find_sample_min_std(
                        B_specimens[sample])
                    sample_acceptable_min, sample_acceptable_max = find_sample_min_max_interpretation(
                        B_specimens[sample], accept_new_parameters)
                    sample_int_interval_uT = sample_acceptable_max - \
                        sample_acceptable_min
                    sample_int_interval_perc = 100 * \
                        ((sample_acceptable_max - sample_acceptable_min)
                    ...
                    if best_std <= (accept_new_parameters['sample_int_sigma_uT']) or 100 * (best_std / best_mean) <= accept_new_parameters['sample_int_sigma_perc']:
                        ...
                        #  String=String + "%.2f,"%(BB)
                            ...
                            thellier_optimizer_master_file.write(
                                "-I- 'best mean' pass. frac=%f; beta=%f  sample %s: mean=%f, std=%f, accepted_interval=%f \n" %
                            ...
                            Optimizer_STDEV_OPT[
                                Key][
                                sample][
                                'sample_int_uT'] = best_mean
                            Optimizer_STDEV_OPT[
                                Key][
                                sample][
                                'sample_int_sigma_uT'] = best_std
                            Optimizer_STDEV_OPT[Key][sample][
                                'sample_int_sigma_perc'] = 100 * (best_std / best_mean)
                            Optimizer_STDEV_OPT[
                                Key][
                                sample][
                                'sample_int_interval_uT'] = best_std
                            Optimizer_STDEV_OPT[Key][sample][
                                'sample_int_interavl_perc'] = 100 * (best_std / best_mean)
                            if sample in samples_expected_intensity.keys():
                                Optimizer_STDEV_OPT[
                                    Key][
                                    sample][
                                    'sample_int_uT_diff_from_expected_uT'] = abs(
                                    best_mean -
                                    samples_expected_intensity[
                                        sample])
                                Optimizer_STDEV_OPT[Key][sample]['sample_int_uT_diff_from_expected_perc'] = 100 * \
                                    (abs(best_mean -
                                     samples_expected_intensity[sample]) / best_mean)
                        else:
                            thellier_optimizer_master_file.write(
                                "-I- 'best mean' fail on accepted_interval. frac=%f; beta=%f  sample %s: mean=%f, std=%f, accepted_interval=%f \n" %
                                (frac, beta, sample, best_mean, best_std, sample_int_interval_uT))
                    else:
                        thellier_optimizer_master_file.write(
                            "-I- 'best mean' fail on sigma. frac=%f; beta=%f  sample %s: mean=%f, std=%f, accepted_interval=%f \n" %
                            (frac, beta, sample, best_mean, best_std, sample_int_interval_uT))
    ...
    for frac in frac_range:
        ...
        for beta in beta_range:
            ...
            tmp = sorted(Optimizer_STDEV_OPT[Key].keys())
                ...
                if sample in Optimizer_STDEV_OPT[Key].keys():
                    if 'sample_int_uT_diff_from_expected_uT' in Optimizer_STDEV_OPT[Key][sample].keys():
                        max_sample_accuracy_uT = max(
                            max_sample_accuracy_uT,
                            Optimizer_STDEV_OPT[Key][sample]['sample_int_uT_diff_from_expected_uT'])
                ...
                if sample in Optimizer_STDEV_OPT[Key].keys():
                    if 'sample_int_uT_diff_from_expected_perc' in Optimizer_STDEV_OPT[Key][sample].keys():
                        max_sample_accuracy_perc = max(
                            max_sample_accuracy_perc,
                            Optimizer_STDEV_OPT[Key][sample]['sample_int_uT_diff_from_expected_perc'])
    ...
    for function in optimization_functions:
        ...
        if "study_sample_n" in function:
            ...
            img = imshow(
                ...
                cmap=discrete_jet,
                extent=(frac_range[0] - delta_f,
                        frac_range[-1] + delta_f,
                        beta_range[-1] + delta_s,
                        beta_range[0] - delta_s))
        else:
            img = imshow(
                optimization_functions_matrices[function],
                interpolation='nearest',
        ...
        Ax.set_aspect('auto')
        savefig(
            "%s/consistency_test/pdf/optimization_function_%i" %
            (WD, Fig_counter) + ".pdf")
        savefig(
            "%s/consistency_test/svg/optimization_function_%i" %
            (WD, Fig_counter) + ".svg")
        close()
        Fig_counter += 1
    ...
    h, m = divmod(m, 60)
    logfile.write(
        "-I- runtime from start (hh:mm:ss) " + "%d:%02d:%02d\n" %
        (h, m, s))
