...
def main():
    ...
    # sets depth scale to meters composite depth (as opposed to meters below
    # sea floor)
    ...
    BulkDepths = []
    for rec in AniData:
        samprecs = pmag.get_dictitem(
            Samps,
            'er_sample_name',
            rec['er_sample_name'].upper(),
            'T')  # look for depth record for this sample
        sampdepths = pmag.get_dictitem(
            samprecs,
            depth_scale,
            '',
            'F')  # see if there are non-blank depth data
        if dmax != -1:
            sampdepths = pmag.get_dictitem(
                sampdepths,
                depth_scale,
                dmax,
                'max')  # fishes out records within depth bounds
            sampdepths = pmag.get_dictitem(
                sampdepths,
                depth_scale,
                dmin,
        ...
        if len(sampdepths) > 0:  # if there are any....
            ...
            if isbulk:  # if there are bulk data
                ...
                chis = pmag.get_dictitem(
                    ...
                    '',
                    'F')  # get the non-zero values for this specimen
    ...
    # make an array
    Ss = numpy.array([s1, s4, s5, s4, s2, s6, s5, s6, s3]).transpose()
    # and re-shape to be n-length array of 3x3 sub-arrays
    Ts = numpy.reshape(Ss, (len(Ss), 3, -1))
    for k in range(len(Depths)):
        # get the sorted eigenvalues and eigenvectors
        tau, Evecs = pmag.tauV(Ts[k])
        # convert to inclination of the minimum eigenvector
        v3 = pmag.cart2dir(Evecs[2])[1]
        V3Incs.append(v3)
        Tau1.append(tau[0])
        Tau2.append(tau[1])
        Tau3.append(tau[2])
