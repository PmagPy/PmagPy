...
def main():
    ...
    backup = 0
    # can skip everything and just plot one specimen with bounds e,b
    specimen = ""
    if '-h' in sys.argv:
        print main.__doc__
        sys.exit()
    ...
    meas_file, geo, tilt, ask, samp_file = dir_path + \
        '/magic_measurements.txt', 0, 0, 0, dir_path + '/er_samples.txt'
    if '-f' in sys.argv:
    ...
    if len(PriorRecs) == 0:
        if verbose:
            print "starting new file ", inspec
    for Rec in PriorRecs:
        ...
        if "magic_method_codes" in Rec.keys():
            ...
            for meth in tmp:
                methods.append(meth)
            if 'DE-FM' in methods:
                # this won't be imported but helps
                Rec['calculation_type'] = 'DE-FM'
            ...
            # default is to assume a best-fit line
            Rec['calculation_type'] = 'DE-BFL'
    #
    ...
    while k < len(sids):
        ...
        if verbose and s != "":
            print s, k, 'out of ', len(sids)
    #
    #  collect info for the PmagSpecRec dictionary
    #
        s_meas = pmag.get_dictitem(
            meas_data,
            'er_specimen_name',
            s,
            'T')  # fish out this specimen
        s_meas = pmag.get_dictitem(
            s_meas,
            'magic_method_codes',
            'Z',
            'has')  # fish out zero field steps
        if len(s_meas) > 0:
            for rec in s_meas:  # fix up a few things for the output record
                PmagSpecRec[
                    "magic_instrument_codes"] = rec[
                    "magic_instrument_codes"]  # copy over instruments
    ...
    #
            ...
            if "T" in units:
                PmagSpecRec[
                    "magic_method_codes"] = PmagSpecRec[
                    "magic_method_codes"] + ":LP-DIR-AF"
            if "K" in units:
                PmagSpecRec[
                    "magic_method_codes"] = PmagSpecRec[
                    "magic_method_codes"] + ":LP-DIR-T"
            if "J" in units:
                PmagSpecRec[
                    "magic_method_codes"] = PmagSpecRec[
                    "magic_method_codes"] + ":LP-DIR-M"
    #
    ...
    #
            if len(CurrRecs) == 0:  # check if already in
                ...
                if inspec != "":
                    ...
                    precs = pmag.get_dictitem(
                        PriorRecs,
                        'er_specimen_name',
                        s,
                        'T')  # get all the prior recs with this specimen name
                    precs = pmag.get_dictitem(
                        precs,
                        'magic_method_codes',
                        'LP-DIR',
                        'has')  # get the directional data
                    PriorRecs = pmag.get_dictitem(
                        PriorRecs,
                        'er_specimen_name',
                        s,
                        'F')  # take them all out of prior recs
               # get the ones that meet the current coordinate system
                    for prec in precs:
                        ...
                        for key in prec.keys():
                            CurrRec[key] = prec[key]
                        CurrRecs.append(CurrRec)  # put in CurrRecs
                        method_codes = CurrRec[
                            "magic_method_codes"].replace(
                            " ",
                            "").split(
                            ':')
                        calculation_type = 'DE-BFL'
                        if 'DE-FM' in method_codes:
                            calculation_type = 'DE-FM'
                        if 'DE-BFP' in method_codes:
                            calculation_type = 'DE-BFP'
                        if 'DE-BFL-A' in method_codes:
...
#        #
        ...
        #
                    ...
                    for rec in datablock:
                        d_geo, i_geo = pmag.dogeo(
                            rec[1], rec[2], float(orient["sample_azimuth"]), float(orient["sample_dip"]))
                        geoblock.append(
                            [rec[0],
                             d_geo,
                             i_geo,
                        ...
                        if tilt == 1 and "sample_bed_dip" in orient.keys() and float(orient['sample_bed_dip']) != 0:
                            d_tilt, i_tilt = pmag.dotilt(
                                d_geo, i_geo, float(orient["sample_bed_dip_direction"]), float(orient["sample_bed_dip"]))
                            tiltblock.append(
                                [rec[0],
                                 d_tilt,
                                 i_tilt,
    ...
    # set the end pca point to last point  if not set
                if e == 0 or e > len(plotblock) - 1:
                    e = len(plotblock) - 1
                if angle == "":
                    angle = plotblock[0][1]  # rotate to NRM declination
                title = s + '_s'
                if geo == 1 and tilt == 0 and noorient != 1:
                    title = s + '_g'
                if tilt == 1 and noorient != 1:
                    title = s + '_t'
                    ...
                    for prec in CurrRecs:
                        ...
                        if calculation_type != "":
                            ...
                            for j in range(len(datablock)):
                                if data[j][0] == float(prec["measurement_step_min"]):
                                    beg_pca = j
                                if data[j][0] == float(prec["measurement_step_max"]):
                                    end_pca = j
    ...
    #
                ...
                for plotrec in plotblock:
                    if units == 'T' and verbose:
                        print '%s: %i  %7.1f %s  %8.3e %7.1f %7.1f %s' % (plotrec[5], recnum, plotrec[0] * 1e3, " mT", plotrec[3], plotrec[1], plotrec[2], plotrec[6])
                    if units == "K" and verbose:
                        print '%s: %i  %7.1f %s  %8.3e %7.1f %7.1f %s' % (plotrec[5], recnum, plotrec[0] - 273, ' C', plotrec[3], plotrec[1], plotrec[2], plotrec[6])
                    if units == "J" and verbose:
                        print '%s: %i  %7.1f %s  %8.3e %7.1f %7.1f %s' % (plotrec[5], recnum, plotrec[0], ' J', plotrec[3], plotrec[1], plotrec[2], plotrec[6])
                    if 'K' in units and 'T' in units:
                        if plotrec[0] >= 1. and verbose:
                            print '%s: %i  %7.1f %s  %8.3e %7.1f %7.1f %s' % (plotrec[5], recnum, plotrec[0] - 273, ' C', plotrec[3], plotrec[1], plotrec[2], plotrec[6])
                        if plotrec[0] < 1. and verbose:
                            print '%s: %i  %7.1f %s  %8.3e %7.1f %7.1f %s' % (plotrec[5], recnum, plotrec[0] * 1e3, " mT", plotrec[3], plotrec[1], plotrec[2], plotrec[6])
                    recnum += 1
                ...
                """
                            ...
                            if ans == 'q':
                                if changeM == 1:
                                    ans = raw_input(
                                        'Save changes to magic_measurements.txt? y/[n] ')
                                    if ans == 'y':
                                        pmag.magic_write(
                                            meas_file,
                                            meas_data,
                            ...
                            if ans == 'c':
                                ...
                                if tilt == 0 and geo == 1:
                                    print "You  are currently viewing geographic  coordinates "
                                if tilt == 1 and geo == 1:
                                    print "You  are currently viewing stratigraphic  coordinates "
                                if tilt == 0 and geo == 0:
                                    print "You  are currently viewing sample coordinates "
                                print "\n Which coordinate system do you wish to view? "
                                coord = raw_input(
                                    " <Return>  specimen, [g] geographic, [t] tilt corrected ")
                                if coord == "g":
                                    geo, tilt = 1, 0
                                if coord == "t":
                                    geo = 1
                                ...
                                if geo == 1 and sfile == "":
                                    samp_file = raw_input(
                                        " Input er_samples file for sample orientations [er_samples.txt] ")
                                    if samp_file == "":
                                        samp_file = "er_samples.txt"
                                    samp_data, file_type = pmag.magic_read(
                                        samp_file)
                                    if file_type != 'er_samples':
                                        print file_type
                            ...
                            if ans == 'h':
                                k -= 1
                                angle = raw_input(
                                    "Enter desired  declination for X axis 0-360 ")
                                angle = float(angle)
                                if angle == 0:
                                    angle = 0.001
                                s = sids[k]
                            ...
                            if ans == 'e':
                                ...
                                recnum = 0
                                for plotrec in plotblock:
                                    if plotrec[0] <= 200 and verbose:
                                        print '%s: %i  %7.1f %s  %8.3e %7.1f %7.1f ' % (plotrec[5], recnum, plotrec[0] * 1e3, " mT", plotrec[3], plotrec[1], plotrec[2])
                                    if plotrec[0] > 200 and verbose:
                                        print '%s: %i  %7.1f %s  %8.3e %7.1f %7.1f ' % (plotrec[5], recnum, plotrec[0] - 273, ' C', plotrec[3], plotrec[1], plotrec[2])
                                    recnum += 1
                                answer = raw_input(
                                    'Enter index of point to change from bad to good or vice versa:  ')
                                try:
                                    ind = int(answer)
                                    meas_data = pmag.mark_dmag_rec(
                                        s,
                                        ind,
                                        meas_data)
                                    changeM = 1
                            ...
                            if ans == 'b':
                                ...
                                while GoOn == 0:
                                    ...
                                    ct = raw_input(
                                        'Enter new Calculation Type: best-fit line,  plane or fisher mean [l]/p/f :  ')
                                    if ct == "" or ct == "l":
                                        direction_type = "l"
                                        calculation_type = "DE-BFL"
                                        GoOn = 1
                                ...
                                if calculation_type != 'DE-FM':
                                    PmagSpecRec["specimen_mad"] = '%7.1f ' % (
                                        mpars["specimen_mad"])
                                    PmagSpecRec["specimen_alpha95"] = ""
                                else:
                                    PmagSpecRec["specimen_alpha95"] = '%7.1f ' % (
                                        mpars["specimen_alpha95"])
                                    PmagSpecRec["specimen_mad"] = ""
                                PmagSpecRec["specimen_n"] = '%i ' % (
                                    mpars["specimen_n"])
                                PmagSpecRec[
                                    "specimen_direction_type"] = direction_type
                                # redundant and won't be imported - just for
                                # convenience
                                PmagSpecRec[
                                    "calculation_type"] = calculation_type
                                method_codes = PmagSpecRec[
                                ...
                                if len(method_codes) != 0:
                                    ...
                                    for meth in method_codes:
                                        ctype = meth.split('-')
                                        if 'DE' not in ctype:
                                            # don't include old direction
                                            # estimation methods
                                            methstring = methstring + \
                                                ":" + meth
                                methstring = methstring + \
                                    ':' + calculation_type
                                PmagSpecRec[
                                ...
                                if calculation_type != 'DE-FM':
                                    if units == 'K':
                                        ...
                                        if float(PmagSpecRec['measurement_step_min']) < 1.0:
                                            min = float(
                                                PmagSpecRec['measurement_step_min']) * 1e3
                                        else:
                                            min = float(
                                                PmagSpecRec['measurement_step_min']) - 273
                                        if float(PmagSpecRec['measurement_step_max']) < 1.0:
                                            max = float(
                                                PmagSpecRec['measurement_step_max']) * 1e3
                                        else:
                                            max = float(
                                                PmagSpecRec['measurement_step_max']) - 273
...
#
                                    ...
                                    if len(CurrRecs) > 0:
                                        replace = raw_input(
                                            " [0] add new component, or [1] replace existing interpretation(s) [default is replace] ")
                                        if replace == "1" or replace == "":
                                            CurrRecs = []
                                            PmagSpecRec[
                                                'specimen_comp_name'] = 'A'
                                            CurrRecs.append(PmagSpecRec)
                                        else:
                                            print 'These are the current component names for this specimen: '
                                            for trec in CurrRecs:
                                                print trec['specimen_comp_name']
                                            compnum = raw_input(
                                                "Enter new component name: ")
                                            PmagSpecRec[
                                                'specimen_comp_name'] = compnum
                        ...
                        for key in ZED.keys():
                            files[key] = "LO:_" + locname + '_SI:_' + PmagSpecRec['er_site_name'] + '_SA:_' + \
                                PmagSpecRec['er_sample_name'] + '_SP:_' + \
                                s + '_CO:_' + coord + \
                                    '_TY:_' + key + '_.' + fmt
            ...
            k += 1  # skip record - not enough data
    if changeM == 1:
