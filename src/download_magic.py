        ...
        while LN < len(File):
            ...
            if line[:4] == ">>>>" and len(Recs) > 0:
                if filenum == 0:
                    ...
                    outfile = dir_path + "/" + \
                        file_type.strip() + '_' + str(filenum) + '.txt'
                NewRecs = []
                for rec in Recs:
                ...
                # sort out zeq_specimens and thellier_specimens
                if file_type == 'pmag_specimens' and 'magic_measurements.txt' in File and 'measurement_step_min' in File and 'measurement_step_max' in File:
    ...
    if len(locs) > 0:  # at least one location
        for loc in locs:
            ...
            for f in type_list:
                ...
                if 'results' not in f:
                    lrecs = pmag.get_dictitem(
                        recs,
                        'er_location_name',
                        loc['er_location_name'],
                        'T')
                    if len(lrecs) > 0:
                        pmag.magic_write(
                            lpath +
