    ...
    for spec in prior_spec_data:
        ...
        if 'DIR' in meths:  # DE-BFL, DE-BFP or DE-FM
            ...
            if 'specimen_comp_name' in spec.keys() and spec['specimen_comp_name'] != "" and spec['specimen_comp_name'] != " ":
                ...
                comp_name = string.uppercase[
                    specs.count(spec['er_specimen_name']) - 1]
            ...
            if zredo == "":
                zredo = open(zfile, "w")
            outstring = '%s %s %s %s %s \n' % (
                spec["er_specimen_name"],
                calculation_type,
                spec["measurement_step_min"],
                spec["measurement_step_max"],
                comp_name)
            ...
            outstring = '%s %i %i \n' % (
                spec["er_specimen_name"],
                float(spec["measurement_step_min"]),
                float(spec["measurement_step_max"]))
            if outstring not in outstrings:
