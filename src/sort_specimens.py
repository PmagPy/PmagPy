...
# get list of specimens in file, components, coordinate systems available
    ...
    for spec in prior_spec_data:
        if spec['er_specimen_name'] not in specs:
            specs.append(spec['er_specimen_name'])
        if 'specimen_comp_name' not in spec.keys():
            spec['specimen_comp_name'] = 'A'
        if 'specimen_tilt_correction' not in spec.keys():
            spec['tilt_correction'] = '-1'  # assume specimen coordinates
        if spec['specimen_comp_name'] not in comps:
            comps.append(spec['specimen_comp_name'])
        if spec['specimen_tilt_correction'] not in coords:
            coords.append(spec['specimen_tilt_correction'])
