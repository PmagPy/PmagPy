...
def main():
    ...
    for loc in dirlist:
        ...
        if 'er_samples.txt' in filelist:  # find coordinate systems
            # read in data
            samps, file_type = pmag.magic_read('er_samples.txt')
        ...
        if 'magic_measurements.txt' in filelist:  # start with measurement data
            ...
            # read in data
            data, file_type = pmag.magic_read('magic_measurements.txt')
            if loc == './':
            ...
            MZrecs = pmag.get_dictitem(
                ...
                'LT-M-Z',
                'has')  # get all none blank method codes
            Drecs = pmag.get_dictitem(
                data,
                'measurement_dec',
                '',
                'F')  # get all dec measurements
            Irecs = pmag.get_dictitem(
                data,
                'measurement_inc',
                '',
                'F')  # get all dec measurements
            ...
            for key in Mkeys:
                # get intensity data
                Mrecs = pmag.get_dictitem(data, key, '', 'F')
                if len(Mrecs) > 0:
                    break
            ...
            if len(pmag.get_dictitem(data, 'magic_method_codes', 'LP-HYS', 'has')) > 0:
        ...
        if 'pmag_results.txt' in filelist:  # start with measurement data
            ...
            # read in data
            data, file_type = pmag.magic_read('pmag_results.txt')
            if loc == './':
                data = pmag.get_dictitem(
                    data,
                    'er_location_names',
                    ':',
                    'has')  # get all the concatenated location names from data file
            ...
            SiteDIs = pmag.get_dictitem(
                ...
                'data_type',
                'i',
                'T')  # only individual results - not poles
        ...
        if 'rmag_hysteresis.txt' in filelist:  # start with measurement data
            ...
            # read in data
            data, file_type = pmag.magic_read('rmag_hysteresis.txt')
            if loc == './':
                data = pmag.get_dictitem(
                    data,
                    'er_location_name',
    ...
    # if 'er_sites.txt' in filelist: # start with measurement data
        ...
        if 'rmag_anisotropy.txt' in filelist:
            ...
            # read in data
            data, file_type = pmag.magic_read('rmag_anisotropy.txt')
            if loc == './':
                data = pmag.get_dictitem(
                    data,
                    'er_location_name',
