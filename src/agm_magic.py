...
# minor loops are LP-M  - do this in measurements_methods function
    if meth == 'LP-HYS':
        recnum = 0
        # this is LP-IMAG
        while float(MeasRecs[recnum]['measurement_lab_field_dc']) < float(MeasRecs[recnum + 1]['measurement_lab_field_dc']) and recnum + 1 < len(MeasRecs):
            MeasRecs[recnum]['magic_method_codes'] = 'LP-IMAG'
            MeasRecs[
                recnum][
                'magic_experiment_name'] = MeasRecs[
                recnum][
