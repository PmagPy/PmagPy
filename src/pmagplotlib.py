...
def gaussfunc(y, ybar, sigma):
    ...
    erf = 1.0 - numpy.exp(-x * x) * t * (.254829592 - t *
                                         (.284496736 - t * (1.421413741 - t * (1.453152027 - t * 1.061405429))))
    erf = abs(erf)
    sign = x / abs(x)
...
def plotQQunf(fignum, D, title):
    ...
    Mu = v * (numpy.sqrt(n) - 0.567 + (1.623 / (numpy.sqrt(n))))
    pylab.axis([0, 1., 0., 1.])
    bounds = pylab.axis()
    notestr = 'N: ' + '%i' % (n)
...
def plotQQexp(fignum, I, title):
    ...
    Me = (ds - (0.2 / n)) * (numpy.sqrt(n) + 0.26 + (0.5 / (numpy.sqrt(n))))
...
def plotZED(ZED, datablock, angle, s, units):
    ...
    if AngleX[-1] == 0:
        AngleX[-1] = 0.01
    pylab.text(
        AngleX[-1] + (AngleX[-1] / abs(AngleX[-1])) * .1,
        AngleY[-1] + (AngleY[-1] / abs(AngleY[-1])) * .1,
        'X')
    norm = 1
    if units == "U":
        norm = 0
...
def plotNP(fignum, indata, s, units):
    global globals
    first_Z, first_I, ptrm_check, ptrm_tail = indata[
        0], indata[1], indata[2], indata[3]
    pylab.figure(num=fignum)
    pylab.clf()
    if not isServer:
        pylab.figtext(.02, .01, version_num)
...
def plotSHAW(SHAW, shawblock, zijdblock, field, s):
    ...
    NRM, TRM, ARM1, ARM2 = shawblock[0], shawblock[
        1], shawblock[2], shawblock[3]
    TRM_ADJ = shawblock[4]
...
def plotB(Figs, araiblock, zijdblock, pars):
    ...
    for rec in zijdblock:
        if rec[0] == pars["measurement_step_min"]:
            Dir.append((rec[1] - angle, rec[2], rec[3] / zijdblock[0][3]))
        if rec[0] == pars["measurement_step_max"]:
            Dir.append((rec[1] - angle, rec[2], rec[3] / zijdblock[0][3]))
    for drec in Dir:
...
#
    sy = []
    sy.append(
        (pars["specimen_b"] * ax[0] + pars["specimen_ytot"] / first_Z[0][3]))
    sy.append(
        (pars["specimen_b"] * ax[1] + pars["specimen_ytot"] / first_Z[0][3]))
...
#
    ...
    for plotrec in datablock:
        ...
        if plotrec[key + '_direction_type'] == 'p':
            GCblock.append(
                (float(plotrec[key + "_dec"]),
                 float(plotrec[key + "_inc"])))
...
#
    Xcirc, Ycirc = [], []
    Da95, Ia95 = pmag.circ(
        float(fpars["dec"]), float(fpars["inc"]), float(fpars["alpha95"]))
    for k in range(len(Da95)):
        XY = pmag.dimap(Da95[k], Ia95[k])
        Xcirc.append(XY[0])
        Ycirc.append(XY[1])
...
# calculate points on the ellipse
        ...
        R = numpy.sqrt(1. - abs(elli[2])) / \
            (numpy.sqrt(elli[0] ** 2 + elli[1] ** 2))
        if elli[2] < 0:
...
def plotHDD(HDD, B, M, s):
    ...
    hpars, deltaM, Bdm = plotHYS(HDD['hyst'], B, M, s)
    DdeltaM = []
    Mhalf = ""
    for k in range(2, len(Bdm)):
        # differnential
        DdeltaM.append(abs(deltaM[k] - deltaM[k - 2]) / (Bdm[k] - Bdm[k - 2]))
...
def plotHPARS(HDD, hpars, sym):
    ...
    X, Y = [], []
    X.append(0)
    Y.append(float(hpars['hysteresis_mr_moment'])
             / float(hpars['hysteresis_ms_moment']))
...
def plotANIS(ANIS, Ss, iboot, ihext, ivec, ipar,
             title, plt, comp, vec, Dir, nb):
    imeas, bpars, hpars = 1, [], []
...
#
    if iboot == 1:
        ...
        Tmean, Vmean, Taus, BVs = pmag.s_boot(Ss, ipar, nb)
...
# scale data
    ...
    if NLpars['banc'] > 0:
        pylab.plot([0, NLpars['best'] * 1e6],
                   [0, NLpars['banc_npred'] / Mp[-1]], 'b--')
        pylab.plot(
            [NLpars['best'] * 1e6,
             NLpars['banc'] * 1e6],
            [NLpars['banc_npred'] / Mp[-1],
             NLpars['banc_npred'] / Mp[-1]],
            'r--')
        pylab.plot(
            [NLpars['best'] * 1e6],
        ...
        pylab.plot([0, NLpars['best'] * 1e6],
                   [0, NLpars['best_npred'] / Mp[-1]], 'b--')
        pylab.plot([0, NLpars['best'] * 1e6],
                   [0, NLpars['best_npred'] / Mp[-1]], 'bd')
...
def addBorders(Figs, titles, border_color, text_color):
    ...
    for key in Figs.keys():
        ...
        border_ax.text(
            (4 / fig.get_figwidth()) *
            0.015, 0.03, now.strftime("%d %B %Y, %I:%M:%S %p"),
            horizontalalignment='left',
            verticalalignment='top',
            color=text_color,
            size=10)
        border_ax.text(0.5, 0.98, plot_title,
                       ...
                       size=20)
        border_ax.text(
            1 - (4 / fig.get_figwidth()) *
            0.015, 0.03, 'http://earthref.org/MAGIC',
...
def plotMAP(fignum, lats, lons, Opts):
    ...
    if Opts['proj'] in ExPar:
        ...
        m = Basemap(
            llcrnrlon=Opts['lonmin'],
            llcrnrlat=Opts['latmin'],
            urcrnrlat=Opts['latmax'],
            urcrnrlon=Opts['lonmax'],
            projection=Opts['proj'],
            lat_0=Opts['lat_0'],
            lon_0=Opts['lon_0'],
            lat_ts=0.,
            resolution=Opts['res'],
            boundinglat=Opts['boundinglat'])
    if 'details' in Opts.keys():
        ...
        if Opts['details']['ocean'] == 1:
            m.drawlsmask(
                land_color=rgba_land,
                ocean_color=rgba_ocean,
                lsmask_lats=None)
    if Opts['pltgrid'] == 0.:
        ...
        if Opts['proj'] in ExMer or Opts['proj'] == 'lcc':
            ...
            g = Opts['gridspace']
            latmin, lonmin = g * \
                int(Opts['latmin'] / g), g * int(Opts['lonmin'] / g)
            latmax, lonmax = g * \
                int(Opts['latmax'] / g), g * int(Opts['lonmax'] / g)
            circles = numpy.arange(
                latmin - 2. * Opts['padlat'],
                latmax + 2. * Opts['padlat'],
                Opts['gridspace'])
            meridians = numpy.arange(
                lonmin - 2. * Opts['padlon'],
                lonmax + 2. * Opts['padlon'],
                Opts['gridspace'])
    if Opts['pltgrid'] >= 0:
...
def plotEQcont(fignum, DIblock):
    ...
    for rec in DIblock:
        ...
        # from Collinson 1983
        R = numpy.sqrt(1. - X[2]) / (numpy.sqrt(X[0] ** 2 + X[1] ** 2))
        XY.append([X[0] * R, X[1] * R])
    radius = (3. / (numpy.sqrt(numpy.pi * (9. + float(counter))))) + \
        0.01  # radius of the circle
