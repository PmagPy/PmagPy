        ...
        if renew == 1:
            ...
            if '#' in description:
                ...
                while c < len(description) - 1:  # look for first pound sign
                    newdesc = newdesc + description[c]
                    c += 1
                # edit out old comment about orientations
                rec['sample_description'] = newdesc
...
# good spec, now get orientation....
                    ...
                    if orient['sample_azimuth'] != "":
                        ...
                        for key in spec.keys():
                            rec[key] = spec[key]
                        rec['dec'], rec['inc'] = pmag.dogeo(float(spec['specimen_dec']), float(
                            spec['specimen_inc']), float(orient['sample_azimuth']), float(orient['sample_dip']))
...
# first test wrong direction of drill arrows (flip drill direction in
# opposite direction and re-calculate d,i
                        d, i = pmag.dogeo(float(spec['specimen_dec']), float(spec['specimen_inc']), float(
                            spec['sample_azimuth']) - 180., -float(spec['sample_dip']))
                        XY = pmag.dimap(d, i)
                        pmagplotlib.plotXY(
                            EQ['eqarea'],
                            [XY[0]],
...
# now spin around specimen's z
                        ...
                        for incr in range(0, 360, 5):
                            d, i = pmag.dogeo(
                                float(spec['specimen_dec']) + incr, float(spec['specimen_inc']), float(spec['sample_azimuth']), float(spec['sample_dip']))
                            XY = pmag.dimap(d, i)
                            if i >= 0:
                                X_d.append(XY[0])
                                Y_d.append(XY[1])
