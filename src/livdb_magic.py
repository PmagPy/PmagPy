        ...
        for dir_name in DIRS_data.keys():
            ...
            for files in os.listdir(DIRS_data[dir_name]["path"]):
                if files.endswith(".livdb"):
                    ...
                    Data = {}
                    header_codes = [
                        'Sample code', 'Sample Dip', 'Sample Dec', 'Height', 'Position', 'Thickness', 'Unit Dip', 'Unit Dip Direction', 'Site Latitude',
                    ...
                    meas_codes = [
                        'Treatment (aka field)', 'Microwave Power', 'Microwave Time', 'moment_X', 'moment_Y', 'moment_Z', 'Mass', 'Applied field Intensity', 'Applied field Dec',
                    ...
                    while continue_reading:
                        ...
                        while this_specimen == True:
                            ...
                            # header consists of  fields seperated by spaces
                            # and "|"
                    ...
                    # del(this_line_data[15:])
                            ...
                            Experiment_Type = tmp_header_data[
                                'Experiment Type']
                    ...
                    for specimen in specimens_list:
                        ...
                        for Experiment_Type in Experiment_Types_list:
                            ...
                            supported_experiments = [
                                'MW-PI-OT+', 'MW-PI-P', 'MW-PI-C', 'MW-PI-C+', 'MW-PI-C++', 'MW-PI-IZZI', 'MW-PI-IZZI+', 'MW-PI-IZZI++', 'MW-PI-A', 'MW-PI-A+',
                            ...
                            if Experiment_Type in supported_experiments:
                                ...
                                for i in range(len(Data[specimen][Experiment_Type]['meas_data'])):
                                    ...
                                    # ignore the first, and take only the
                                    # second one

                                    if i < (len(Data[specimen][Experiment_Type]['meas_data']) - 2):
                                        Repeating_measurements = True
                                        for key in [
                                                "Treatment (aka field)", "Microwave Power", "Microwave Time", "AF Demag/Remag Peak Field", "Applied field Intensity",
                                    ...
                                    MagRec[
                                        "er_specimen_name"] = header_line[
                                        'Sample code']
                                    MagRec["er_sample_name"] = self.get_sample_name(
                                        MagRec["er_specimen_name"],
                                        DIRS_data[dir_name]['sample_naming_convenstion'])
                                    MagRec["er_site_name"] = self.get_site_name(
                                        MagRec["er_sample_name"],
                                        DIRS_data[dir_name]['site_naming_convenstion'])
                                    MagRec[
                                        'er_location_name'] = DIRS_data[
                                        dir_name][
                                        'location_name']
                                    MagRec[
                                        'er_analyst_mail_names'] = header_line[
                                        'Name of measurer']
                                    MagRec["magic_instrument_codes"] = header_line[
                                        'Magnetometer name'] + ":" + header_line['Demagnetiser name']
                                    ...
                                    CART = array(
                                        [1e-9 * float(meas_line['moment_X']),
                                         1e-9 * float(meas_line['moment_Y']),
                                            1e-9 * float(meas_line['moment_Z'])])  # Am^2
                                    DIR = cart2dir(CART)
                                    ...
                                    mm = date[0]
                                    hour = meas_line[
                                        'Measurement Time'].strip("\"")
                                    MagRec["measurement_date"] = yyyy + \
                                        ':' + mm + ":" + dd + ":" + hour

                                    # lab field data: distinguish between PI
                                    # experiments to AF/Thermal
                                    if Experiment_Type == "TH-D" or Experiment_Type == "AF-D":
                                        ...
                                        MagRec[
                                            ...
                                            labfield)
                                        MagRec[
                                            "treatment_dc_field_phi"] = meas_line[
                                            'Applied field Dec']
                                        MagRec[
                                            "treatment_dc_field_theta"] = meas_line[
                                            'Applied field Inc']

                                    # treatment (MW or Thermal)
                                    if "MW-" in Experiment_Type:
                                        MagRec[
                                            'treatment_mw_power'] = meas_line[
                                            'Microwave Power']
                                        MagRec[
                                            'treatment_mw_time'] = meas_line[
                                            'Microwave Time']
                                        MagRec[
                                            'treatment_mw_integral'] = meas_line[
                                            'Microwave Power Integral']
                                        MagRec['measurement_description'] = "Step Type" + "-" + \
                                        ...
                                        MagRec['treatment_temp'] = "%.2f" % (
                                            float(meas_line['TH Demag/Remag Peak Temperature']) + 273)
                                        treatment = meas_line[
                                            'TH Demag/Remag Peak Temperature']
                                    ...
                                    if Experiment_Type in [
                                            'MW-PI-C', 'MW-PI-C+', 'MW-PI-C++', 'MW-PI-IZZI', 'MW-PI-IZZI+', 'MW-PI-IZZI++', 'MW-PI-A', 'MW-PI-A+',
                                        ...
                                        # check Step Type in the file and the
                                        # deduced step from the program
                                    ...
                                    MagRec[
                                        "magic_method_codes"] = lab_treatment + ":" + lab_protocols_string
                                    MagRec[
                                        "magic_experiment_name"] = specimen + ":" + lab_protocols_string
                                    MagRecs.append(MagRec)
                                    measurement_running_number += 1
                                    headers = MagRec.keys()
                                    for key in headers:
                                        if key not in magic_measurements_headers:
                                            magic_measurements_headers.append(
                                                key)
                    ...
                    for specimen in specimens_list:
                        ...
                        for Experiment_Type in Experiment_Types_list:
                            ...
                            if Experiment_Type in supported_experiments:
                                ...
                                ErRec[
                                    'specimen_dip'] = header_line[
                                    'Sample Dip']
                                if float(ErRec['specimen_dip']) == 99:
                                    ErRec['specimen_dip'] = ""
                                ErRec[
                                    'specimen_azimuth'] = header_line[
                                    'Sample Dec']
                                if float(ErRec['specimen_azimuth']) == 999:
                                    ErRec['specimen_azimuth'] = ""
                                ErRec[
                                    'specimen_height'] = header_line[
                                    'Height']
                                ErRec[
                                    'specimen_description'] = header_line[
                                    'Specimen/Experiment Comment']
                                try:
                                    ErRec['specimen_volume'] = "%f" % float(
                                        header_line['Sample Volume']) * 1e-6
                                except:
                                    ErRec['specimen_volume'] = ""

                                ErRec[
                                    'specimen_density'] = header_line[
                                    'Sample Density']
