...
is the percent cooling rate factor to apply to specimens from this sample, DA-CR-XX is the method code
    ...
    orient_file, samp_file, or_con, corr = dir_path + \
        "/orient.txt", dir_path + "/er_samples.txt", "1", "1"
    ...
    #
        ...
        if "IGSN" in OrRec.keys():
            ...
            MagRec["sample_igsn"] = ""
        MagRec["sample_height"], MagRec["sample_bed_dip_direction"], MagRec[
            "sample_bed_dip"] = "", "", ""
...
#
        ...
        if corr == "1" and MagRec['sample_azimuth'] != "":
        ...
        if "bedding_dip_direction" in OrRec.keys():
            if OrRec["bedding_dip_direction"] != "" and BedCorr == 1:
                ...
                MagRec[
                    "sample_bed_dip_direction"] = OrRec[
                    'bedding_dip_direction']
        else:
            MagRec["sample_bed_dip_direction"] = '0'
        if average_bedding != "0":
            BPs.append(
                [float(MagRec["sample_bed_dip_direction"]),
                 float(MagRec["sample_bed_dip"]) - 90.,
                    1.])
    ...
    # work on the site stuff too
        ...
        if "image_name" in OrRec.keys():
            ...
            for image in images:
                if image != "" and image not in imagelist:
                    ...
                    if len(looks) > k:
                        ImageRec['er_image_description'] = "Look direction: " + \
                            looks[k]
                    elif len(looks) >= 1:
                        ImageRec['er_image_description'] = "Look direction: " + \
                            looks[-1]
                        ...
                        ImageRec[
                            'er_image_description'] = "Look direction: unknown"
                    if len(photographers) > k:
                        ImageRec[
                            'er_photographer_mail_names'] = photographers[
                            k]
        ...
        if sample not in samplelist:
            ...
            if MagRec['sample_azimuth'] != "":  # assume magnetic compass only
                MagRec[
                    'magic_method_codes'] = MagRec[
                    'magic_method_codes'] + ':SO-MAG'
                MagRec[
                    'magic_method_codes'] = MagRec[
                    'magic_method_codes'].strip(
                    ":")
    ...
    #
            ...
            if "shadow_angle" in OrRec.keys() and OrRec["shadow_angle"] != "":
                if delta_u == "":
                    delta_u = raw_input(
                        "Enter hours to SUBTRACT from time for  GMT: [0] ")
                    if delta_u == "":
                        delta_u = "0"
                SunRec, sundata = {}, {}
...
#                    MagRec["sample_time_zone"]='GMT+'+delta_u+' hours'
    ...
    #
            ...
            if "prism_angle" in OrRec.keys() and OrRec["prism_angle"] != "":
                GPSRec = {}
                for key in MagRec.keys():
            ...
            if "GPS_Az" in OrRec.keys() and OrRec["GPS_Az"] != "":
                GPSRec = {}
                for key in MagRec.keys():
                    GPSRec[key] = MagRec[key]  # make a copy of MagRec
    ...
    for rec in SampOuts:
        if average_bedding != "0":
            ...
            Samps.append(rec)
    for rec in SampRecs:
        # overwrite prior for this sample
        if rec['er_sample_name'] not in samplelist:
            Samps.append(rec)
    for rec in SiteRecs:
        # overwrite prior for this sample
    ...
    for rec in ImageRecs:
        # overwrite prior for this sample
        if rec['er_image_name'] not in imagelist:
