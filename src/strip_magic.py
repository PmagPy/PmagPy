...
def main():
    ...
    if "magic_method_codes" in Results[0].keys():
        for rec in Results:
                ...
                for xkeys in keys:
                    if key in xkeys:
                        for ResRec in Results:
                            if ResRec[key] != "":
                                # only plot something if there is something to
                                # plot!
                                Xplots.append(key)
                                break
            for keys in Y_keys:
...
#        isign=1.
    ...
    XY.sort()
    title = ""
    if "er_locations_names" in Results[0].keys():
        title = Results[0]["er_location_names"]
    if "er_locations_name" in Results[0].keys():
