...
def main():
    ...
    while cont == 1:
        try:
            Int.append(
                int(raw_input(" Enter desired treatment step interval: <return> to quit ")))
            Top.append(
                int(raw_input(" Enter upper bound for this interval: ")))
        except:
            cont = 0
    low, k, iz = 100, 0, 0
    vline = '\t%s\n' % (
    ...
    for k in range(len(Top)):
        for t in range(low, Top[k] + Int[k], Int[k]):
            if iz == 0:
                ...
                f.write(
                    '\t %s \t %s' %
                    ('I', str(t) + '.' + str(iz)))  # infield after zero field first
                f.write(vline)
                f.write(hline)
                f.write(
                    '\t %s \t %s' %
                    ('T', str(t) + '.' + str(3)))  # print second zero field (tail check)
                f.write(vline)
                f.write(hline)
            elif iz == 1:
                f.write(
                    '\t %s \t %s' %
                    ('I', str(t) + '.' + str(iz)))  # infield first step
                f.write(vline)
                f.write(hline)
                iz = 0
                f.write(
                    '\t %s \t %s' %
                    ('Z', str(t) + '.' + str(iz)))  # zero field step (after infield)
                f.write(vline)
                f.write(hline)
        try:
