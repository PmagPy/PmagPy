...
def main():
    ...
    if '-L' in sys.argv:
        pltL = 0
    if '-S' in sys.argv:
        pltS = 0  # don't plot the bulk measurements at all
    if '-D' in sys.argv:
        pltD = 0
        pcol -= 1
        pel -= 1
    ...
    if pltS:  # plot the bulk measurement data
        m1 = pmag.get_dictitem(
            Meas,
            'magic_method_codes',
            method,
            'has')  # fish out the desired method code
        if method == 'LT-T-Z':
            m2 = pmag.get_dictitem(
                m1,
                'treatment_temp',
                str(step),
            ...
            if dmax == -1 or float(rec['core_depth']) < dmax and float(rec['core_depth']) > dmin:
                ...
                if method == "LP-X":
                    ...
                    if norm == 1 and pltM == 1:
                        Ints.append(
                            float(rec[meas_key]) / float(rec['specimen_weight']))
            if len(SSucs) > 0:
                maxSuc = max(SSucs)
                minSuc = min(SSucs)
            if len(Ints) > 1:
            ...
            if sum_file != "":
                for core in Cores:
                    ...
                    if depth > dmin and depth < dmax:
                        ...
                        if pel == plt:
                            pylab.text(360, depth + tint, core['Core Label'])
            if pel == plt:
    ...
    if pltI == 1:
        ...
        if pltL == 1:
            pylab.plot(Incs, Depths, 'k')
        if len(Incs) > 0:
            pylab.plot(Incs, Depths, sym, markersize=size)
        if len(Incs) == 0 and pltL == 1 and len(SIncs) > 0:
            pylab.plot(SIncs, SDepths, 'k')
        if len(SIncs) > 0:
            pylab.plot(SIncs, SDepths, Ssym, markersize=Ssize)
        ...
        if sum_file != "":
            for core in Cores:
                ...
                if depth > dmin and depth < dmax:
                    if pel == plt:
                        pylab.text(90, depth + tint, core['Core Label'])
    ...
    if pltM == 1 and len(Ints) > 0 or len(SInts) > 0:
        ...
        if logit == 0:
            ...
            if pltL == 1 and len(Ints) > 0:
                pylab.plot(Ints, Depths, 'k')
            if len(Ints) > 0:
                pylab.plot(Ints, Depths, sym, markersize=size)
            ...
            if sum_file != "":
                for core in Cores:
                    depth = float(core['Core Top (m)'])
                    pylab.plot(
                        [0, maxInt * 10 ** pow + .1], [depth, depth], 'b--')
                    if depth > dmin and depth < dmax:
                        pylab.text(
                            maxInt * 10 ** pow - .2 * maxInt * 10 ** pow,
                            depth + tint,
                            core['Core Label'])
    ...
    if suc_file != "" or len(SSucs) > 0:
        ...
        if len(Susc) > 0:
            if pltL == 1:
                pylab.plot(Susc, Sus_depths, 'k')
            if logit == 0:
                pylab.plot(Susc, Sus_depths, sym, markersize=size)
            if logit == 1:
                pylab.semilogx(Susc, Sus_depths, sym, markersize=size)
        if len(SSucs) > 0:
            if logit == 0:
                pylab.plot(SSucs, SDepths, sym, markersize=size)
            if logit == 1:
                pylab.semilogx(SSucs, SDepths, sym, markersize=size)
        if sum_file != "":
            for core in Cores:
                depth = float(core['Core Top (m)'])
                if logit == 0:
