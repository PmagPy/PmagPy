...
def main():
    ...
    #
    A, Vgps, Pvgps = 180., [], []
    # get all non-blank vgp latitudes
    VgpRecs = pmag.get_dictitem(data, 'vgp_lat', '', 'F')
    # get all non-blank vgp longitudes
    VgpRecs = pmag.get_dictitem(VgpRecs, 'vgp_lon', '', 'F')
    # get VGPs (as opposed to averaged)
    SiteRecs = pmag.get_dictitem(VgpRecs, 'data_type', 'i', 'T')
    SiteRecs = pmag.get_dictitem(
        SiteRecs,
        coord_key,
        coord,
            ...
            if anti == 1:
                if 90. - abs(float(rec['vgp_lat'])) <= cutoff and float(rec['average_k']) >= kappa:
                    if float(rec['vgp_lat']) < 0:
                        ...
                        rec['vgp_lon'] = '%7.1f' % (
                            float(rec['vgp_lon']) - 180.)
                    Vgps.append(rec)
                    Pvgps.append(
                        [float(rec['vgp_lon']), float(rec['vgp_lat'])])
    ...
    if spin == 0:  # do transformation to pole
        ...
        for vgp in Vgps:
            vlon, vlat = pmag.dodirot(
                float(vgp['vgp_lon']), float(vgp['vgp_lat']), ppars['dec'], ppars['inc'])
            vgp['vgp_lon'] = vlon
            vgp['vgp_lat'] = vlat
            vgp['average_k'] = "0"
