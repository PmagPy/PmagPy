...
# read in data
    ...
    #
        meas_meth = []
        # copy of vital stats to PmagSpecRec from first spec record
        for rec in meas_data:
            if rec["er_specimen_name"] == s:
                ...
                if len(rec["magic_instrument_codes"]) > len(inst_code):
                    ...
                    # copy over instruments
                    PmagSpecRec["magic_instrument_codes"] = inst_code
                break
    #
            ...
            #
                if skip == 0 and orient["sample_azimuth"] != "" and orient["sample_dip"] != "":
                    d_geo, i_geo = pmag.dogeo(
                        SpecRec[1], SpecRec[2], orient["sample_azimuth"], orient["sample_dip"])
                    SpecRec[1] = d_geo
                    SpecRec[2] = i_geo
