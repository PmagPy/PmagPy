...
def main():
    ...
    meas_file, spec_file = dir_path + \
        "/magic_measurements.txt", dir_path + "/er_specimens.txt"
    out_file = meas_file
    MeasRecs, SpecRecs = [], []
    OutRecs = []
    if "-h" in sys.argv:
            ...
            for spec in Specs:
                if spec['er_specimen_name'] == rec['er_specimen_name']:
                    if 'specimen_weight' in spec.keys() and spec['specimen_weight'] != "":
                        rec['measurement_magn_mass'] = '%e' % (
                            float(rec['measurement_magn_moment']) / float(spec['specimen_weight']))
                    if 'specimen_volume' in spec.keys() and spec['specimen_volume'] != "":
                        rec['measurement_magn_volume'] = '%e' % (
                            float(rec['measurement_magn_moment']) / float(spec['specimen_volume']))
                    break
        if 'measurement_magn_volume' not in rec.keys():
            rec['measurement_magn_volume'] = ''
        if 'measurement_magn_mass' not in rec.keys():
            rec['measurement_magn_mass'] = ''
        OutRecs.append(rec)
