...
def spitout(line):
    rec = line.split()
    sundata = {}
    # assign first column to delta_u key in sundec dictionary
    sundata["delta_u"] = rec[0]
    ...
    hours = rec[6]
    min = rec[7]
    # put together the date the way dosundec wants it.
    sundata["date"] = year + ":" + month + ":" + day + ":" + hours + ":" + min
    sundata["shadow_angle"] = rec[8]
    # print out the output from sundec (the magnetic declination)
    dec = pmag.dosundec(sundata)
    # print out the output from sundec (the magnetic declination)
    print '%7.1f' % (dec)
    return dec
...
def main():
 ...
 """
    ...
    if '-f' in sys.argv:
        ...
        sys.exit()
    if '-i' in sys.argv:
        while True:  # repeat this block until program killed
            sundata = {}  # dictionary with sundata in it
            print (
                "Time difference between Greenwich Mean Time (hrs to ADD to GMT for local time): ")
            try:
