...
#
    ...
    for file in filelist:  # parse each file
        if file[-3:].lower() == 'srm':
            ...
            date = mmddyy[2] + ':' + mmddyy[0] + \
                ":" + mmddyy[1] + ':' + datestamp[1]
            ...
            while True:
                ...
                rec = line.replace('\n', '').split(',')  # list of data
                if len(rec) > 2:
                    MeasRec, SampRec = {}, {'core_depth': '0', 'er_sample_name':
                                            '0', 'er_site_name': '0', 'er_location_name': 'location'}
                    ...
                    for item in rec:
                        ...
                        if 'intensity_w_tray_w_bkgrd' in items[0]:
                            ...
                            MeasRec['measurement_magn_moment'] = '%8.3e' % (
                                float(items[1]) * vol)
                        MeasRec['magic_instrument_codes'] = inst
                        if 'offset' in items[0]:
                            depth = '%7.3f' % (
                                float(sect - 1) * 1.5 + float(items[1]))
