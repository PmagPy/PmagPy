...
def main():
    ...
    if '-WD' in args:
        ind = args.index('-WD')
        dir_path = args[ind + 1]
    meas_file, pmag_file, mk_file = dir_path + "/" + "magic_measurements.txt", dir_path + \
        "/" + "zeq_specimens.txt", dir_path + "/" + "zeq_redo"
...
# now get down to bidness
    ...
    #
        ...
        if len(spec) == 0:
            ...
            # copy of vital stats to PmagSpecRec from first spec record in
            # demag block
            for rec in spec:
                skip = 1
    ...
    #
    #
        # fish out the demag data for this specimen
        datablock, units = pmag.find_dmag_rec(s, spec)
    #
...
#                    datablock=avedata
       ...
       # find top priority orientation method
                ...
                if az_type not in method_codes:
                ...
                for spec in mkspec:
                    if spec[0] == s:
                        ...
                        for l in range(len(datablock)):
                            if datablock[l][0] == beg:
                                beg_pca = l
                            if datablock[l][0] == end:
                                end_pca = l
                        if geo == 1 and tilt == 0:
                            mpars = pmag.domean(
                                geoblock,
                        ...
                        if mpars["specimen_direction_type"] == "Error":
                            ...
                            if geo == 1:
                                CompMeths.append("DA-DIR-GEO")
                            if tilt == 1:
                                CompMeths.append("DA-DIR-TILT")
