...
# set defaults
    ...
    # this skips VADM calculation altogether, when get_model_lat=1, uses
    # present day
    get_model_lat = 0
    fmt = 'svg'
    dir_path = "."
...
# get command line stuff
    ...
    if "-aD" in args:
        Daverage = 1  # average by sample directions
    if "-aI" in args:
        Iaverage = 1  # average by sample intensities
    if "-aC" in args:
        Caverage = 1  # average all components together ???  why???
    if "-pol" in args:
        polarity = 1  # calculate averages by polarity
    if '-xD' in args:
        noDir = 1
    if '-xI' in args:
        noInt = 1
    ...
    if "-p" in args:
        ...
        if noDir == 0:  # plot by site - set up plot window
            ...
            # I don't know why this has to be here, but otherwise the first
            # plot never plots...
    ...
    # use default criteria (if nocrit set, then get really loose criteria as
    # default)
    else:
...
#
    ...
    comment, orient = "", []
    samples, sites = [], []
    # run through the data filling in missing keys and finding all components,
...
# fill in missing fields, collect unique sample and site names
        ...
        if 'specimen_int' not in rec.keys():
            rec['specimen_int'] = ''
        if 'specimen_comp_name' not in rec.keys() or rec['specimen_comp_name'] == "":
        ...
        if len(nocorrection) > 0 and len(SpecInts) > 0:
            for cor in nocorrection:
                SpecInts = pmag.get_dictitem(
                    ...
                    'magic_method_codes',
                    cor,
                    'not')  # exclude the corrections not specified for inclusion
            ...
            if len(SampDir) > 0:  # there are some directions
                for coord in coords:  # step through desired coordinate systems
                    ...
                    if len(CoordDir) > 0:
                        if Caverage == 0:  # look component by component
                            for comp in Comps:
                                CompDir = pmag.get_dictitem(
                                    ...
                                    'specimen_comp_name',
                                    comp,
                                    'T')  # get all directions from this component
                                if len(CompDir) > 0:  # there are some
                                    PmagSampRec = pmag.lnpbykey(
                                        CompDir,
                                        'sample',
                                        'specimen')  # get a sample average from all specimens
                                    # decorate the sample record
                                    PmagSampRec["er_location_name"] = CompDir[
                                        0]['er_location_name']
                                    PmagSampRec["er_site_name"] = CompDir[
                                        0]['er_site_name']
                                    ...
                                    if len(site_height) > 0:
                                        ...
                                        PmagSampRec["sample_height"] = site_height[
                                            0]['site_height']
                                    ...
                                    PmagSampRec[
                                        ...
                                        CompDir,
                                        'er_specimen_name')  # get a list of the specimen names used
                                    PmagSampRec[
                                        'magic_method_codes'] = pmag.get_list(
                                        CompDir,
                                        'magic_method_codes')  # get a list of the methods used
                                    ...
                                    if len(kill) == 0:
                                        ...
                                        # if sample level VGP info desired, do
                                        # that now
                                        if vgps == 1:
                        ...
                        # average all components together  basically same as
                        # above
                        if Caverage == 1:
                            PmagSampRec = pmag.lnpbykey(
                                CoordDir,
                                'sample',
                                'specimen')
                            PmagSampRec["er_location_name"] = CoordDir[
                                0]['er_location_name']
                            PmagSampRec["er_site_name"] = CoordDir[
                                0]['er_site_name']
                            PmagSampRec["er_sample_name"] = samp
                            ...
                            PmagSampRec[
                                'sample_comp_name'] = pmag.get_list(
                                CoordDir,
                                'specimen_comp_name')  # get components used
                            PmagSampRec[
                                'er_specimen_names'] = pmag.get_list(
                                CoordDir,
                                'er_specimen_name')  # get specimne names averaged
                            PmagSampRec[
                                'magic_method_codes'] = pmag.get_list(
                                CoordDir,
        ...
        if Iaverage == 1:  # average by sample if desired
            ...
            if len(SampI) > 0:  # there are some
                PmagSampRec = pmag.average_int(
                    SampI,
                    'specimen',
                    'sample')  # get average intensity stuff
                ...
                if vgps == 1 and get_model_lat != 0 and PmagSampRec != {}:
                    if get_model_lat == 1:  # use sample latitude
                        ...
                        if PmagResRec != {}:
                            PmagResRec[
                                'magic_method_codes'] = PmagResRec[
                                'magic_method_codes'] + ":IE-MLAT"
                    if PmagResRec != {}:
                        PmagResRec[
                            'er_specimen_names'] = PmagSampRec[
                            'er_specimen_names']
                        PmagResRec[
                            'er_sample_names'] = PmagSampRec[
                            'er_sample_name']
                        PmagResRec['pmag_criteria_codes'] = 'ACCEPT'
                        PmagResRec[
                            'average_int_sigma_perc'] = PmagSampRec[
                            'sample_int_sigma_perc']
                        PmagResRec[
                            'average_int_sigma'] = PmagSampRec[
                            'sample_int_sigma']
                        PmagResRec[
                            'average_int_n'] = PmagSampRec[
                            'sample_int_n']
            ...
            if Caverage == 0:  # do component wise averaging
                for comp in Comps:
                    ...
                    if len(siteD) > 0:
                        ...
                        PmagSiteRec["er_location_name"] = siteD[
                            0]['er_location_name']
                        PmagSiteRec["er_site_name"] = siteD[0]['er_site_name']
                        PmagSiteRec['site_tilt_correction'] = coords[-1]
                        PmagSiteRec[
                            'site_comp_name'] = pmag.get_list(
                            siteD,
                            key +
                            '_comp_name')
                        if Daverage == 1:
                            PmagSiteRec[
                                'er_sample_names'] = pmag.get_list(
                                siteD,
                                'er_sample_name')
                        else:
                            PmagSiteRec[
...
# determine the demagnetization code (DC3,4 or 5) for this site
    ...
    # now decorate each dictionary some more, and calculate VGPs etc. for
    # results table
    for PmagSiteRec in PmagSites:
        ...
        if 'site_n_lines' in PmagSiteRec.keys() and 'site_n_planes' in PmagSiteRec.keys() and PmagSiteRec['site_n_lines'] != "" and PmagSiteRec['site_n_planes'] != "":
            if int(PmagSiteRec["site_n_planes"]) > 0:
                PmagSiteRec[
                    "magic_method_codes"] = PmagSiteRec[
            ...
            if len(kill) == 0:
                ...
                PmagResRec[
                    "er_location_names"] = PmagSiteRec[
                    "er_location_name"]
                if Daverage == 1:
                    PmagResRec[
                        "er_sample_names"] = PmagSiteRec[
                        "er_sample_names"]
                else:
                    PmagResRec[
                        "er_specimen_names"] = PmagSiteRec[
                        "er_specimen_names"]
                PmagResRec[
                    "tilt_correction"] = PmagSiteRec[
                ...
                site_height = pmag.get_dictitem(
                    ...
                    'T')
                if len(site_height) > 0:
                    PmagResRec["average_height"] = site_height[
                        0]['site_height']
                ...
                # assign polarity based on angle of pole lat to spin axis - may
                # want to re-think this sometime
                if polarity == 1:
    ...
    if noInt != 1 and nositeints != 1:
        for site in sites:  # now do intensities for each site
            ...
            if len(Ints) > 0:  # there are some
                ...
                # if site by site examination requested - print this site out
                # to the screen
                if plotsites == 1:
                    for rec in Ints:
