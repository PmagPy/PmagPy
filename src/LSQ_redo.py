...
# now find all the data for this specimen in measurements
        ...
        if len(datablock) > 0:
            ...
            while incl[-1] > len(datablock) - 1:
                # don't include measurements beyond what is in file
                del incl[-1]
            if len(incl) != incl_int:
                'converting calculation type to best-fit line'
            meths0 = datablock[incl[0]][
                'magic_method_codes'].replace(" ", "").split(':')
            meths1 = datablock[incl[-1]
                               ]['magic_method_codes'].replace(" ", "").split(':')
    ...
    # collect the rest of the measurement data not already included
    for s in meas_data:
        if s['er_specimen_name'] not in specs:
            MeasRecs.append(s)
    # write out annotated measurements
