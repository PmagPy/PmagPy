...
def main():
    ...
    def calculate_aniso_parameters(B, K):
        ...
        S_bs = S_bs / trace
        s1, s2, s3, s4, s5, s6 = S_bs[0], S_bs[
            1], S_bs[2], S_bs[3], S_bs[4], S_bs[5]
    ...
    rmag_results_header = [
        'er_specimen_names', 'er_sample_names', 'er_site_names', 'anisotropy_type', 'magic_method_codes', 'magic_experiment_names', 'result_description', 'anisotropy_t1', 'anisotropy_t2', 'anisotropy_t3', 'anisotropy_ftest', 'anisotropy_ftest12', 'anisotropy_ftest23',
    ...
    for specimen in specimens:
        ...
        if 'atrmblock' in Data[specimen].keys():
            ...
            if len(baselines) != 0:
                ...
                baseline = array(
                    [mean(baselines[:, 0]), mean(baselines[:, 1]), mean(baselines[:, 2])])
            ...
            if not Reject_specimen:
                ...
                Data_anisotropy[specimen]['ATRM'][
                    'er_site_name'] = atrmblock[0]['er_site_name']
                Data_anisotropy[
                    specimen][
                    'ATRM'][
        ...
        if 'aarmblock' in Data[specimen].keys():
            ...
            Data_anisotropy[specimen]['AARM'][
                'er_sample_name'] = aarmblock[0]['er_sample_name']
            Data_anisotropy[specimen]['AARM'][
                'er_site_name'] = aarmblock[0]['er_site_name']
        ...
        for TYPE in TYPES:
            ...
            for i in range(len(rmag_anistropy_header)):
                try:
                    ...
                    String = String + \
                        "%f" % (
                            Data_anisotropy[specimen][TYPE][rmag_anistropy_header[i]]) + '\t'
            rmag_anisotropy_file.write(String[:-1] + "\n")
            ...
            Data_anisotropy[
                specimen][
                TYPE][
                'er_specimen_names'] = Data_anisotropy[
                specimen][
                TYPE][
                'er_specimen_name']
            Data_anisotropy[
                specimen][
                TYPE][
                'er_sample_names'] = Data_anisotropy[
                specimen][
                TYPE][
                'er_sample_name']
            Data_anisotropy[
