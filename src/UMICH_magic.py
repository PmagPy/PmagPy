...
#
        ...
        for line in input.readlines():
            ...
            if len(line) > 2:
                ...
                if len(code1) > 1:
                    ...
                    # takes care of awkward format with bubba and flo being
                    # different
                    MagRec["measurement_positions"] = code1[7]
                if user == "":
                    user = code1[5]
                if code1[2][-1] == 'C':
                    demag = "T"
    ...
    for spec in specs:  # gather all demag types for this specimen
        ...
        expname = spec
        if "LT-AF-Z" in meths:
            expname = expname + ':LP-DIR-AF'
        if "LT-T-Z" in meths:
            expname = expname + ':LP-DIR-T'
        for rec in SpecRecs:
