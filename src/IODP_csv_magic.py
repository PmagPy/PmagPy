...
# csv_file=args[ind+1] # original
    ...
    for file in filelist:  # parse each file
        if file[-3:].lower() == 'csv':
            ...
            if "Inclination background + tray corrected  (\xc2\xb0)" in keys:
            ...
            if "Declination background + tray corrected (\xc2\xb0)" in keys:
...
# Maintain backward compatibility for the ever-changing LIMS format (Argh!)
                    ...
                    if "Last Tray Measurment" in InRec.keys() and "Discrete" in InRec['Last Tray Measurement'] or 'dscr' in csv_file:
                        specimen = expedition + '-' + location + '-' + \
                            InRec['Core'] + InRec[type] + "-" + InRec['Section'] + \
                            '-' + InRec['Section Half'] + \
                            '-' + InRec[interval_key]
                    else:  # mark as continuous measurements
                        specimen = expedition + '-' + location + '-' + \
                            InRec['Core'] + InRec[type] + "_" + InRec['Section'] + \
                            InRec['Section Half'] + '-' + InRec[interval_key]
...
# set up measurement record - default is NRM
                    ...
                    if 'Sample Area (cm?)' in InRec.keys() and InRec['Sample Area (cm?)'] != "":
                        volume = InRec['Sample Area (cm?)']
                    if InRec['Run Number'] != "":
                        run_number = InRec['Run Number']
                    # date time is second line of file
                    datestamp = InRec['Test Changed On'].split()
                    ...
                    if InRec[demag_key] != "0":
                        ...
                        treatment_value = float(
                            InRec[demag_key].strip('"')) * 1e-3
                        # AF demag in treat mT => T
                        MagRec["treatment_ac_field"] = treatment_value
                    if InRec['Treatment Type'] != "":
                        if 'Alternating Frequency' in InRec['Treatment Type']:
                            ...
                            # convert mT => T
                            treatment_value = float(
                                InRec['Treatment Value']) * 1e-3
                            # AF demag in treat mT => T
                            MagRec["treatment_ac_field"] = treatment_value
                        elif 'Thermal' in InRec['Treatment Type']:
                            MagRec['magic_method_codes'] = 'LT-T-Z'
                            # measured on shipboard Schonstedt thermal
                            # demagnetizer
                            inst = inst + ':ODP-TDS'
                            # convert C => K
                            treatment_value = float(
                                InRec['Treatment Value']) + 273
                            #
                            MagRec[
                                "treatment_temp"] = '%8.3e' % (
                                treatment_value)
