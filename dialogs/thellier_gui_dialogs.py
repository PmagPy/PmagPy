...
class convert_generic_files_to_MagIC(wx.Frame):
    ...
    def InitUI(self):
        ...
        for i in range(self.max_files):
            ...
            exec(command)
            command = "self.add_file_button_%i =  wx.Button(self.panel, id=-1, label='add',name='add_%i')" % (
                i, i)
            exec(command)
            ...
            exec(command)
            command = "bSizer0_%i.Add(wx.StaticText(pnl,label=('%i  '[:2])),wx.ALIGN_LEFT)" % (
                i, i + 1)
            exec(command)

            command = "bSizer0_%i.Add(self.file_path_%i,wx.ALIGN_LEFT)" % (
                i, i)
            exec(command)
            command = "bSizer0_%i.Add(self.add_file_button_%i,wx.ALIGN_LEFT)" % (
                i, i)
            exec(command)
        ...
        self.experiments_names = ['IZZI', 'IZ', 'ZI',
                                  'ATRM 6 positions', 'cooling rate', 'NLT']
        bSizer1.AddSpacer(5)
        for i in range(self.max_files):
        ...
        for i in range(self.max_files):
            ...
            exec(command)
            command = "bSizer2_%i.Add(self.file_info_Blab_%i ,wx.ALIGN_LEFT)" % (
                i, i)
            exec(command)
            command = "bSizer2_%i.Add(self.file_info_Blab_dec_%i,wx.ALIGN_LEFT)" % (
                i, i)
            exec(command)
            command = "bSizer2_%i.Add(self.file_info_Blab_inc_%i,wx.ALIGN_LEFT)" % (
                i, i)
            exec(command)
    ...
    def cart2dir(cart):
        ...
        return array([Decs, Incs, Rs]).transpose()
            ...
            for specimen in list(this_file_data.keys()):
                ...
                for meas_line in this_file_data[specimen]:
                    ...
                    MagRec["er_sample_name"] = self.get_sample_name(
                        MagRec["er_specimen_name"], sample_naming_convenstion)
                    MagRec["er_site_name"] = self.get_site_name(
                        MagRec["er_sample_name"], site_naming_convenstion)
                    ...
                    MagRec['treatment_temp'] = "%.2f" % (
                        float(meas_line['Treatment'].split(".")[0]) + 273.)
                    ...
                    MagRec["treatment_dc_field"] = '%8.3e' % (
                        float(labfield[0]) * 1e-6)
                    MagRec["treatment_dc_field_phi"] = "%.2f" % (
                        float(labfield[1]))
                    MagRec["treatment_dc_field_theta"] = "%.2f" % (
                        float(labfield[2]))
                    MagRec["measurement_standard"] = "u"
                    MagRec["magic_experiment_name"] = MagRec["er_specimen_name"] + \
                        ":" + Magic_lab_protocols[experiment]

                    this_specimen_teratments.append(
                        float(meas_line['Treatment']))
                    # fill in LP and LT
                    ...
                    if experiment in ['IZZI', 'IZ', 'ZI']:
                        ...
                        if float(tr_tr) == 0:
                            ...
                            if float(tr_tr) == 70 or float(tr_tr) == 7:
                                lab_treatment = "LT-PTRM-I"
                            else:
                                lab_treatment = "LT-T-I"
                        ...
                        print(
                            "Dont support yet cooling rate experiment file. Contact rshaar@ucsd.edu")
                    ...
                    MagRec["magic_method_codes"] = lab_treatment + \
                        ":" + lab_protocols_string
                    line_string = ""
                    for i in range(len(magic_headers)):
                        line_string = line_string + \
                            MagRec[magic_headers[i]] + "\t"
                    fout.write(line_string[:-1] + "\n")

        #--
        MSG = "files converted to MagIC format and merged into one file:\n magic_measurements.txt\n "
        dlg1 = wx.MessageDialog(None, caption="Message:",
                                message=MSG, style=wx.OK | wx.ICON_INFORMATION)
        dlg1.ShowModal()
        dlg1.Destroy()
