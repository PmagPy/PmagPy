           ...
           if len(first_Z)<3:
               ...
               if len(tdsrecs)>2: # a TDS experiment
                   ...
                   while mkey=="" and k<len(Mkeys)-1: # find which type of intensity
                       key= Mkeys[k]
                       if key in list(tdsrecs[0].keys()) and tdsrecs[0][key]!="": mkey=key
                       k+=1
                   ...
                   for tdrec in tdsrecs:
                       ...
                       if  'LT-T-I' in meths and Tnorm=="": # found first total TRM 
                           ...
                           tdsblock.append([273,old_div(zijdblock[0][3],Tnorm),1.]) # put in the zero step
                           ...
                           if Tint!="":
                               for zrec in zijdblock:
                                   if zrec[0]==step:  # found matching
                                       tdsblock.append([step,old_div(zrec[3],Tnorm),old_div(Tint,Tnorm)])
                                       break
                   if len(tdsblock)>2: 
