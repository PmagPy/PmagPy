...
def main(**kwargs):
    ...
    con.tables['specimens'].df = pd.DataFrame(SpecRecs)
    con.tables['samples'].df = pd.DataFrame(SampRecs)
    con.tables['sites'].df = pd.DataFrame(SiteRecs)
    con.tables['locations'].df = pd.DataFrame(LocRecs)
    Fixed=pmag.measurements_methods3(MeasRecs,noave)
    con.tables['measurements'].df = pd.DataFrame(Fixed)
