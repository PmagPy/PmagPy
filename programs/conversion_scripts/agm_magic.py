...
def convert(**kwargs):
    ...
    if not spec_file:spec_file=specimen+'_specimens.txt' 
    if not samp_file:samp_file=specimen+'_samples.txt'
    if not site_file:site_file=specimen+'_sites.txt' 
    if not loc_file:loc_file=specimen+'_locations.txt' 
    ...
    if "ASCII" not in Data[0] and fmt!='xy':fmt='new'
