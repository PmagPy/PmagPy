...
61. **Step selection vs. bound editing (Directions, 2026-09-02).** A left click in the step logger now *selects* a step (bold row, rings on all three plots, ↑ ↓ move it) and no longer moves a bound. Bounds are set by tapping a point on the Zijderveld or net, box-selecting a range, the Lower/Upper bound selectors, or `[ ] { }`. Open points: (a) should a plot tap also select the step (it currently only moves a bound / starts a two-tap fit), or the other way round — should taps select and bounds be set only through the selectors and brackets, for one consistent rule? (b) a key to set the selected fit's lower/upper bound *to the selected step* (e.g. `,` `.`) would give the keyboard the full loop: ↑ ↓ to the step, one key to snap the bound. (c) ↑ ↓ act on the Specimen and Means tabs; the Fits and Poles tables could take them too. (d) with a step selected, should the status line report it (step, dec, inc, M), or is the ring enough?

## Intensity merged in (2026-09-02, Yiming Zhang)

62. **The SPD criteria sets, for §49's second default set.** Question 49 asks
    someone to "name the set you want" for a paleointensity default built on
    the Standard Paleointensity Definitions rather than the old `-dcr`
    thresholds. They exist now: `pmagpy.paleointensity.CRITERIA_SETS` holds
    **CCRIT, RCRIT, TTA, TTB** and their modified forms, **PICRIT03**,
    **SELCRIT2**, and *This study* (whatever the directory's own
    `criteria.txt` says), each with its citation and DOI, and each as a list
    of `Criterion(key, operator, threshold)` that `CriteriaSet.evaluate`
    scores against a statistics dict. `data.criteria_table()` writes any of
    them as a MagIC `criteria.txt`. So Metadata's *Add default criteria* could
    offer them by name instead of translating `pmag.default_criteria(0)` —
    my suggestion is **CCRIT** as the paleointensity default (it is what the
    community selects on now, and it is the one Megiddo's own table matches),
    with the others in a dropdown. Want me to wire that into Metadata, or
    should the intensity app stay the only place criteria are chosen?

63. **Two writers on `criteria.txt`** (follows from 7 and 49). The intensity
    app writes `criteria.txt` on export — the set the results were produced
    under, which is provenance rather than configuration — and Metadata now
    edits the same file as a grid. If an analyst sets CCRIT in Intensity,
    exports, then opens Metadata, the grid shows CCRIT and could be edited
    into something the exported results were not scored against. Options:
    (a) Metadata owns the file and Intensity only reads it (then choosing a
    named set in Intensity has to write it, which is the same collision);
    (b) Intensity writes `criteria.txt` and Metadata treats the intensity
    rows as read-only, editing only the directional ones; (c) both write, and
    the row's `software_packages` says which. I lean to (b). Which?

64. **`ipmag` at module scope in the shared core.** `magic_project.py` imports
    `ipmag` inside `download_contribution` for `download_magic`. That is fine
    and the intensity core's Data-Model-2 test now allows it *inside a
    function* — importing an analysis study must never load `ipmag`, which is
    what PmagPy/PmagPy#789 is about, and `test_intensity_environment.py`
    checks the other half. Worth knowing if you ever want to hoist it.

65. **What the intensity app still owes.** Written down so it is visible
    rather than implied: no Windows or Linux run (macOS arm64 only — the
    highest-value thing left, and cheap in CI); BiCEP is validated against
    synthetic known-answer sites and a real site but **not** against Cych
    et al. (2021)'s own 30-site compilation, because that repository's licence
    contradicts itself (CC BY-SA 4.0 in the repo, MIT in the metadata) and I
