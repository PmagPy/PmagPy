"""Assemble the PmagPy Rock Magnetism application."""
...
from .views import TABS, DataView, ExperimentIndex

shell.setup()

ASSETS = os.path.join(os.path.dirname(os.path.abspath(__file__)), "assets")
LOGO = os.path.join(ASSETS, "pmagpy_logo_white.png")
DEFAULT_EXAMPLE = "RMB_oxyhydroxides"


def build_body(session: Session) -> shell.Body:
    """The application's body: side column (dataset, experiment index), one tab per view, status, chooser."""
    dataview = DataView(session)
    index = ExperimentIndex(session)
    ...
    body = shell.Body(info=APP, main=tabs,
                      side=pn.Column(dataview.sidebar(), index.panel(), sizing_mode="stretch_width"),
...
def create_app(directory: str):
    ...
    if session.measurements is None:
