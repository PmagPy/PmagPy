"""
pmagpy.contribution_builder: MagicDataFrame and Contribution over the
Osler, McMurdo and Megiddo example contributions.

Moved here from ``pmagpy_tests/`` (whose package imports wx) so the tests
run in a headless environment.  Paths are taken from the repository, not the
working directory; the few tests that write into it chdir to a scratch
directory first.
"""
import unittest
import os
...
class TestMagicDataFrame(unittest.TestCase):
    ...
    def test_front_and_backfill(self):
        ...
        self.assertEqual(sorted(directions, key=lambda x: -1 if pd.isnull(x) else x)[1:], [135, 135])
        self.assertTrue(pd.isnull(directions).sum() == 1)
    ...
    def test_convert_to_pmag_list(self):
        ...
        for val in df.df.loc['mgq04t1', 'age_low'].values:
            self.assertTrue(pd.isnull(val))                 # None becomes NaN in a str column
...
class TestContribution(unittest.TestCase):
    ...
    def test_propagate_lithology_cols(self):
        ...
        self.assertTrue(pd.isnull(res[0]))
    ...
    def test_propagate_average_up(self):
        ...
        con.tables['sites'].df.loc['mc01', 'lat'] = np.nan
        con.propagate_average_up()
        # fails gracefully?
        con = cb.Contribution(directory, read_tables=['sites', 'samples'],
    ...
    def test_propagate_ages(self):
        ...
        self.con.tables['sites'].df.loc['hz11', 'age'] = np.nan       # pandas 3: no '' in a float column
        # do propagation
