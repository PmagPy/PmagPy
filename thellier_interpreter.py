...
class thellier_auto_interpreter():
    ...
    def run_interpreter(self):
        ...
        # log file
        self.thellier_interpreter_log=open(self.WD+"/"+"/thellier_interpreter//thellier_interpreter.log",'w')
        ...
        for crit in self.specimen_criteria: #+ ["specimen_b"] + ['specimen_cm_x'] + ['specimen_cm_y']:
            String=String+crit+"\t"
        String=String[:-1]+"\n"
        ...
        if self.acceptance_criteria['interpreter_method']['value']=='stdev_opt':
            self.Fout_STDEV_OPT_redo=open(self.WD+"/thellier_interpreter/thellier_interpreter_STDEV-OPT_redo",'w')
            self.Fout_STDEV_OPT_specimens=open(self.WD+"/thellier_interpreter/thellier_interpreter_STDEV-OPT_specimens.txt",'w')
            ...
            if self.acceptance_criteria['average_by_sample_or_site']['value']=='sample':
                self.Fout_STDEV_OPT_samples=open(self.WD+"/thellier_interpreter/thellier_interpreter_STDEV-OPT_samples.txt",'w')
                self.Fout_STDEV_OPT_samples.write(criteria_string)
                self.Fout_STDEV_OPT_samples.write("er_sample_name\tsample_int_n\tsample_int_uT\tsample_int_sigma_uT\tsample_int_sigma_perc\tsample_int_min_uT\tsample_int_min_sigma_uT\tsample_int_max_uT\tsample_int_max_sigma_uT\tsample_int_interval_uT\tsample_int_interval_perc\tWarning\n")
            else:
                self.Fout_STDEV_OPT_sites=open(self.WD+"/thellier_interpreter/thellier_interpreter_STDEV-OPT_sites.txt",'w')
                self.Fout_STDEV_OPT_sites.write(criteria_string)
                self.Fout_STDEV_OPT_sites.write("er_site_name\tsite_int_n\tsite_int_uT\tsite_int_sigma_uT\tsite_int_sigma_perc\tsite_int_min_uT\tsite_int_min_sigma_uT\tsite_int_max_uT\tsite_int_max_sigma_uT\tsite_int_interval_uT\tsite_int_interval_perc\tWarning\n")
        ...
        specimens_list.sort()
        self.thellier_interpreter_log.write("-I- Found %i specimens\n"%(len(specimens_list)))

        #try:
        All_grade_A_Recs={}
        for s in specimens_list:
            self.thellier_interpreter_log.write("-I- doing now specimen %s\n"%s)
            self.Data[s]['pars']={}
            ...
            for t in range(len(temperatures)-1):
                if float(temperatures[t+1])<float(temperatures[t]):
                    self.thellier_interpreter_log.write("-W- Found problem in the temperature order of specimen %s. skipping specimen\n"%(s))
            ...
            for tmin_i in range(len(temperatures)-specimen_int_n+1):
                ...
                for tmax_i in range(tmin_i+specimen_int_n-1,len(temperatures)):
                    ...
                    pars=thellier_gui_lib.get_PI_parameters(self.Data,self.acceptance_criteria,self.preferences,s,tmin,tmax,self.GUI_log,THERMAL,MICROWAVE)
                            ...
                            if "AC_anisotropy_type" in pars.keys():
                                if "AC_WARNING" in pars.keys():
                                    if "TRM" in pars["AC_WARNING"] and pars["AC_anisotropy_type"]== "ATRM" and "alteration" in pars["AC_WARNING"]:
                                        continue
                                    AC_correction_factor_1=max(AC_correction_factor_1,100*abs(1.-pars["Anisotropy_correction_factor"]))
    ...
    def update_files_with_intrepretation(self,Grade_A_sorted,All_grade_A_Recs,sample_or_site,thellier_interpreter_pars):
                            ...
                            for specimen in Grade_A_sorted[sample_or_site].keys():
                                ...
                                for TEMP in All_grade_A_Recs[specimen].keys():
                                    if All_grade_A_Recs[specimen][TEMP]['specimen_int_uT']==thellier_interpreter_pars['stdev_opt_interpretations'][specimen]:
                                        ...
                                        self.Fout_STDEV_OPT_redo.write("%s\t%i\t%i\n"%(specimen,t_min,t_max))
            ...
            if self.acceptance_criteria['interpreter_method']['value']=='bs' or self.acceptance_criteria['interpreter_method']['value']=='bs_par':
               ...
               (sample,sample_n,sample_median,sample_68[0],sample_68[1],sample_95[0],sample_95[1],sample_std,100*(sample_std/sample_median),WARNING)
    ...
    def thellier_interpreter_BS_pars_calc(self,Grade_As):
        ...
        BOOTSTRAP_N=int(self.preferences['BOOTSTRAP_N'])
        Grade_A_samples_BS={} 
        if len(Grade_As.keys()) >= self.acceptance_criteria['sample_int_n']['value']:
            for specimen in Grade_As.keys():
                if specimen not in Grade_A_samples_BS.keys() and len(Grade_As[specimen])>0:
                ...
                if specimen_int_max_slope_diff>self.acceptance_criteria['specimen_int_max_slope_diff']:
                    self.thellier_interpreter_log.write( "-I- specimen %s Failed specimen_int_max_slope_diff\n"%specimen,Grade_A_samples_BS[specimen])
        ...
        thellier_interpreter_pars['bs_n']=len(Grade_A_samples_BS.keys())
